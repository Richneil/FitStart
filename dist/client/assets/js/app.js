...
document.addEventListener("submit",event=>{
  ...
  if(form.id==="review-form"){event.preventDefault();const state=store.get(),draft=structuredClone(state.draftAssessment);draft.metrics=draft.metrics.map(metric=>{const raw=formValue(form,`metric-${metric.id}`),numeric=typeof metric.value==="number";return{...metric,value:raw===""?"":numeric?Number(raw):raw,status:formValue(form,`status-${metric.id}`),available:raw!==""}});const missing=["weight","pbf","smm"].filter(id=>!draft.metrics.find(m=>m.id===id)?.available);if(missing.length)return formError(form,"review","Body weight, body fat percentage, and muscle mass are required.");if(!form.elements.reviewed.checked)return formError(form,"review","Confirm that you reviewed the values against your report.");const commit=()=>{store.confirmAssessment(draft);toast("Report confirmed.");if(previewObjectUrl){URL.revokeObjectURL(previewObjectUrl);previewObjectUrl=null}go("#/context/confirm")};if(state.assessments.length>=2&&!draft.editingAssessmentId)modal({title:"Replace the oldest report?",body:"<p>FITSTART keeps only your two newest reports. Continuing removes the oldest one from this browser.</p>",confirmText:"Replace oldest report",onConfirm:commit});else commit()}
